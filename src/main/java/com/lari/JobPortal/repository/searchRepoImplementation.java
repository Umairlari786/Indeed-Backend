...
public class searchRepoImplementation implements searchRepository{
    ...
    public List<Post> findByText(String text) {
        ...
        MongoDatabase database = client.getDatabase("JobData");
        MongoCollection<Document> collection = database.getCollection("JobReq");
        AggregateIterable<Document> result = collection.aggregate(Arrays.asList(new Document("$search",
                                 new Document("text",
                                    new Document("query", text).append("path", Arrays.asList("skils", "description", "profile")))),
                                 new Document("$sort",
                                new Document("exp", 1L)),
